    ...
    for y in range(maze.height):
        ...
        for x in range(maze.width):
            if pixels[x, y] != WALL:
                ...
                horizontal_paths = (left != WALL or right != WALL)
                vertical_paths = (top != WALL or bottom != WALL)
                turning_point = horizontal_paths and vertical_paths
                agent = [left, right, top, bottom].count(AGENT) == 1
                dead_end = [left, right, top, bottom].count(PATH) == 1

                if turning_point or dead_end or pixels[x,y] == AGENT:
                    node = Node((x, y))
                    if not start_node and (x == 0 or y == 0):
                        start_node = node

                    elif not finish_node and pixels[x,y] == AGENT:
